...
struct rotor_traits<std::complex<T>> {
	...
	static auto calc_w(index_type k, size_type n) -> std::complex<T> {
		return exp(std::complex<T>(0, (-2.0*M_PI*k)/static_cast<T>(n)));	
	}
};
...
struct signal_array_traits {
	...
	using iterator_type = typename SignalArray::iterator_type;
	using index_type = typename SignalArray::index_type;

	static auto begin(const SignalArray& sa) -> iterator_type;
	static auto end(const SignalArray& sa) -> iterator_type;
	static auto size(const SignalArray& sa) -> size_type;
	static auto construct(size_type size) -> SignalArray;
	static auto at(SignalArray& sa, index_type index) -> value_type&;
};

template<class T>
struct signal_array_traits<std::vector<T>> {
	using size_type = size_t; 
	...
	static auto begin(std::vector<T>& sa) -> iterator_type {
		return sa.begin();	
	}

	static auto end(std::vector<T>& sa) -> iterator_type {
		return sa.end();	
	}

	static auto cbegin(std::vector<T>& sa) -> const_iterator_type {
		return sa.cbegin();	
	}

	static auto cend(std::vector<T>& sa) -> const_iterator_type {
		return sa.cend();	
	}

	static auto size(const std::vector<T>& sa) -> size_type {
		return sa.size();	
	}

	static auto construct(size_type size) -> std::vector<T> {
		return std::vector<T>(size);	
	}

	static auto at(const std::vector<T>& sa, index_type index) -> const value_type& {
		return sa.at(index);
	}

	static auto at(std::vector<T>& sa, index_type index) -> value_type& {
		return sa.at(index);
	}
